...
export function StartInfoModal({ open, onClose }: StartInfoModalProps) {
  ...
  return (
    <>
      ...
      />
      <div className="fixed inset-x-0 top-1/2 z-50 mx-4 -translate-y-1/2 rounded-2xl bg-white px-6 py-6 shadow-xl">
        ...
        <div className="space-y-2">
          ...
          <button
            type="button"
            onClick={() => onClose(false)}
            className="h-12 w-full rounded-full border border-gray-200 bg-white text-base font-semibold text-gray-700"
          >
            다음에 할게요
          </button>
        </div>
